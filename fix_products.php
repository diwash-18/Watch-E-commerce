...
// First check if we already have the correct number of products
...
if (mysqli_query($conn, $insert_products)) {
    echo "Products table has been refreshed successfully!<br>";
    ...
    $brand_query = "SELECT brand, COUNT(*) as count FROM products GROUP BY brand ORDER BY brand";
    $result = mysqli_query($conn, $brand_query);
    echo "<br>Products by brand:<br>";
    while ($row = mysqli_fetch_assoc($result)) {
        echo $row['brand'] . ": " . $row['count'] . " products<br>";
    }
} else {
    echo "Error refreshing products table: " . mysqli_error($conn);
}
